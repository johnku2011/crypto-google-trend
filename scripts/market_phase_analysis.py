...
def detect_price_trends(df, price_col, window=3, pct_threshold=10):
    ...
    df['positive_trend'] = df['pct_change'] > pct_threshold
    df['negative_trend'] = df['pct_change'] < -pct_threshold
...
def detect_trend_peaks_troughs(df, trend_col, prominence=5):
    """Detect peaks and troughs in Google Trends data"""
    # Find peaks
    ...
    # Create indicators
    df['trend_peak'] = False
    df['trend_trough'] = False
    
    if len(peak_indices) > 0:
        df.loc[peak_indices, 'trend_peak'] = True
        
    if len(trough_indices) > 0:
        df.loc[trough_indices, 'trend_trough'] = True
    
    return df, peak_indices, trough_indices

def analyze_trend_price_relationship(df, trend_col, price_col, forward_window=3, pct_threshold=10, use_consistent_movement=True):
    """Analyze if trend peaks/troughs indicate future price trends"""
    results = {
    ...
    for idx in peaks.index:
        if idx + forward_window < len(df):
            ...
            if use_consistent_movement:
                ...
                if df.loc[idx+1:idx+forward_window, 'negative_trend'].any():
                    results['peak_followed_by_negative'] += 1
    ...
    for idx in troughs.index:
        if idx + forward_window < len(df):
            ...
            if use_consistent_movement:
                ...
                # Use the original percentage-based definition
                if df.loc[idx+1:idx+forward_window, 'positive_trend'].any():
                    results['trough_followed_by_positive'] += 1
...
def plot_market_phase_analysis(df, trend_col, price_col):
    """Plot the relationship between trend peaks/troughs and price trends"""
    plt.figure(figsize=(15, 10))
    
    # Plot 1: Trend with peaks and troughs
    plt.subplot(2, 1, 1)
    plt.plot(df['Month'], df[trend_col], label=f'{trend_col.capitalize()} Trend')
    
    # Plot peaks
    peaks = df[df['trend_peak']]
    plt.scatter(peaks['Month'], peaks[trend_col], color='red', marker='^', s=100, label='Peaks')
    
    # Plot troughs
    troughs = df[df['trend_trough']]
    plt.scatter(troughs['Month'], troughs[trend_col], color='green', marker='v', s=100, label='Troughs')
    
    plt.title(f'{trend_col.capitalize()} Trend Peaks and Troughs')
    ...
    plt.subplot(2, 1, 2)
    plt.plot(df['Month'], df[price_col], label=f'{trend_col.capitalize()} Price')
    
    # Highlight positive trend phases
    positive_phases = df[df['positive_trend']]
    ...
    plt.savefig(os.path.join(project_root, 'outputs', f'trend_price_analysis_{trend_col}.png'))
    plt.close()

def run_analysis(df, trend_cols=['bitcoin', 'ethereum'], forward_windows=[1, 3, 6], prominence=5, pct_threshold=10, use_consistent_movement=True):
    """Run price trend analysis for multiple assets and time windows"""
    print("\nRunning Trend-Price Analysis...")
    
    all_results = {}
    
    for trend_col in trend_cols:
        print(f"\nAnalyzing {trend_col.capitalize()}:")
        price_col = f'{trend_col}_mean'
        
        # Detect price trends
        df = detect_price_trends(df, price_col, pct_threshold=pct_threshold)
        
        # Detect trend peaks and troughs
        df, peak_indices, trough_indices = detect_trend_peaks_troughs(df, trend_col, prominence=prominence)
        
        # Plot overall analysis
        plot_market_phase_analysis(df, trend_col, price_col)
        ...
        for window in forward_windows:
            ...
            results = analyze_trend_price_relationship(
                df, trend_col, price_col, forward_window=window, 
            ...
            for idx in peaks.index:
                if idx + window < len(df):
                    ...
                    if use_consistent_movement:
                        ...
                        if df.loc[idx+1:idx+window, 'negative_trend'].any():
                            peak_bear_months.append(df.loc[idx, 'Month'])
            ...
            for idx in troughs.index:
                if idx + window < len(df):
                    ...
                    if use_consistent_movement:
                        ...
                        if df.loc[idx+1:idx+window, 'positive_trend'].any():
                            trough_bull_months.append(df.loc[idx, 'Month'])
            ...
            print(f"Trend Peaks followed by Bear Market: {peak_negative_rate:.1f}% ({results['peak_followed_by_negative']}/{results['peak_total']})")
            if peak_bear_months:
                print(f"  Months with peak → bear market: {', '.join(peak_bear_months)}")
            
            print(f"Trend Peaks followed by >{pct_threshold}% Price Drop: {peak_drop_rate:.1f}% ({results['peak_followed_by_drop']}/{results['peak_total']})")
            
            print(f"Trend Troughs followed by Bull Market: {trough_positive_rate:.1f}% ({results['trough_followed_by_positive']}/{results['trough_total']})")
            if trough_bull_months:
                print(f"  Months with trough → bull market: {', '.join(trough_bull_months)}")
            
            print(f"Trend Troughs followed by >{pct_threshold}% Price Rise: {trough_rise_rate:.1f}% ({results['trough_followed_by_rise']}/{results['trough_total']})")
        ...
        all_results[trend_col] = window_results
    
    print("\nTrend-Price Analysis completed.")
